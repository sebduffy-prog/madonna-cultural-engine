...
// ── Blob helpers ──
// Cache the store base URL to avoid list() calls (advanced operations)
let storeBaseUrl = null;
let blobSuspended = false; // Stop retrying if store is suspended

async function blobPut(key, value) {
  if (!HAS_BLOB || blobSuspended) return false;
  try {
    const blob = await import("@vercel/blob");
    const path = `sweet-tooth/${safeKey(key)}.json`;
    ...
    // Cache the store base URL from the first successful put
    if (!storeBaseUrl && result?.url) {
      const idx = result.url.indexOf("/sweet-tooth/");
      if (idx > 0) storeBaseUrl = result.url.substring(0, idx);
    }
    return true;
  } catch (err) {
    if (err.message?.includes("suspended")) {
      blobSuspended = true;
      console.warn("[kv] Blob store suspended — using in-memory cache only");
    } else {
...
async function blobGet(key) {
  if (!HAS_BLOB || blobSuspended) return null;
  try {
    const safe = safeKey(key);
    const path = `sweet-tooth/${safe}.json`;

    // Fast path: if we know the store URL, fetch directly (no list() needed)
    if (storeBaseUrl) {
      const directUrl = `${storeBaseUrl}/${path}`;
      const res = await fetch(directUrl, { signal: AbortSignal.timeout(8000) });
      if (res.ok) return await res.json();
      if (res.status === 404) return null;
      // Non-404 error — fall through to list() as fallback
    }

    // Fallback: use list() to discover the URL (1 advanced operation)
    const blob = await import("@vercel/blob");
    ...
    const url = result.blobs[0].downloadUrl || result.blobs[0].url;

    // Cache the store base URL for future direct fetches
    if (!storeBaseUrl) {
      const idx = url.indexOf("/sweet-tooth/");
      if (idx > 0) storeBaseUrl = url.substring(0, idx);
    }
